...
class Workflow:
    ...
    def _research_step(self, state: ResearchState) -> Dict[str, Any]:
        ...
        if not extracted_tools:
            ...
            fallback_results = []
            if hasattr(search_results, 'data'):
                fallback_results = search_results.data
            elif isinstance(search_results, dict):
                fallback_results = search_results.get('data', [])
            
            tool_names = [
                getattr(getattr(result, "metadata", {}), "title", "Unknown")
        ...
        for tool_name in tool_names:
            ...
            if tool_search_results:
                if hasattr(tool_search_results, 'data'):
                    data = tool_search_results.data
                elif isinstance(tool_search_results, dict):
                    data = tool_search_results.get('data', [])
                else:
                    data = []
                if data:
                    result = data[0]
